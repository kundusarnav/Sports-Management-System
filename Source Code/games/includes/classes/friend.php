...
class Friend{
    ...
    public function is_already_friends($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `friends` WHERE (user_one = :my_id AND user_two = :frnd_id AND game = 'cricket') OR (user_one = :frnd_id AND user_two = :my_id AND game = 'cricket')";
    ...
    public function am_i_the_req_sender($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `friend_request` WHERE sender = ? AND receiver = ? AND game = 'cricket'";
            $stmt = $this->db->prepare($sql);
    ...
    public function is_request_already_sent($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `friend_request` WHERE (sender = :my_id AND receiver = :frnd_id AND game = 'cricket') OR (sender = :frnd_id AND receiver = :my_id AND game = 'cricket')";
            $stmt = $this->db->prepare($sql);
            $stmt->bindValue(':my_id',$my_id, PDO::PARAM_INT);
            $stmt->bindValue(':frnd_id', $user_id, PDO::PARAM_INT);
            $stmt->execute();
    ...
    public function make_pending_friends($my_id, $user_id){
        ...
        try{
            $sql = "INSERT INTO `friend_request`(sender, receiver, game) VALUES(?,?,'cricket')";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$my_id, $user_id]);
    ...
    public function cancel_or_ignore_friend_request($my_id, $user_id){
        try{
            $sql = "DELETE FROM `friend_request` WHERE (sender = :my_id AND receiver = :frnd_id AND game = 'cricket') OR (sender = :frnd_id AND receiver = :my_id AND game = 'cricket')";
            $stmt = $this->db->prepare($sql);
            $stmt->bindValue(':my_id',$my_id, PDO::PARAM_INT);
            $stmt->bindValue(':frnd_id', $user_id, PDO::PARAM_INT);
            $stmt->execute();
    ...
    public function make_friends($my_id, $user_id){
        try{
            $delete_pending_friends = "DELETE FROM `friend_request` WHERE (sender = :my_id AND receiver = :frnd_id AND game = 'cricket') OR (sender = :frnd_id AND receiver = :my_id AND game = 'cricket')";
            ...
            if($delete_stmt->execute()){
                $sql = "INSERT INTO `friends`(user_one, user_two, game) VALUES(?, ?, 'cricket')";
                $stmt = $this->db->prepare($sql);
                $stmt->execute([$my_id, $user_id]);
                header('Location: user_profile.php?id='.$user_id);
                exit;
    ...
    public function delete_friends($my_id, $user_id){
        try{
            $delete_friends = "DELETE FROM `friends` WHERE (user_one = :my_id AND user_two = :frnd_id AND game = 'cricket') OR (user_one = :frnd_id AND user_two = :my_id AND game = 'cricket')";
    ...
    public function request_notification($my_id, $send_data){
        try{
            $sql = "SELECT sender, username, user_image FROM `friend_request` JOIN users ON friend_request.sender = users.id WHERE receiver = ? AND game = 'cricket'";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$my_id]);
            if($send_data){
    ...
    public function get_all_friends($my_id, $send_data){
        try{
            $sql = "SELECT * FROM `friends` WHERE user_one = :my_id OR user_two = :my_id AND game = 'cricket'";
            $stmt = $this->db->prepare($sql);
            $stmt->bindValue(':my_id',$my_id, PDO::PARAM_INT);
            $stmt->execute();
                if($send_data){
